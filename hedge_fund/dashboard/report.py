...
<tr><td>Universe</td><td>Explicit ~50-name 5m list (was 3546 combinatorial clones). Lookbacks in names are bar counts (e.g. <code>dip_24b</code> = 24×5m = 2 hours). <code>daily()</code>/<code>h1()</code>/<code>m5()</code> and MFI are not generated. Empty-pool fallback: <code>PAPER_STRATEGY=sma_stack</code>. Leftover untested names drain 24/7 across cycles: each tournament invocation takes a time-shared slice (not a 30-name sample, not the whole leftover list in one blocking run).</td></tr>
