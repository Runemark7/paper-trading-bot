...
| 2026-09-14 | Same-date later: refill mint densifies 4–7 atom AND stacks on the admit island via small role buckets (REGIME+MOM spine, 0–2 continuation, 0–1 mild dip, 0–1 RSI, optional cheap `near_swing_hi` N≤48 at depth 7). Not a cartesian. Depth 4–5 first, then 6–7. `name_is_parseable` allows up to 7 atoms (`parse_strategy` already ANDs `&`). No HTF×mom×expensive structure. `STRUCTURE_NS` still ≤96. Fail-once stays. No named candlesticks. Static list unchanged. Paper only; OOS gates unchanged. |
