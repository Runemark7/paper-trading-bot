...
class ProtocolAmendmentTests(unittest.TestCase):
    ...
    def test_amendment_2026_09_13_grind_one_pct_and_full_3atoms(self):
        from hedge_fund.trading.constants import (
            DISCOVER_CYCLE_MAX_NAMES,
            DISCOVERY_REFILL_BATCH_SIZE,
            MIN_BACKTEST_SHARPE,
        ...
        text = (REPO / "PROTOCOL.md").read_text()
        self.assertIn("grind 1% mint bases and full short-MA 3-atoms", text)
        self.assertIn("DIP_FILTERS_GRIND", text)
        self.assertIn("MOM_FILTERS_GRIND", text)
        self.assertIn("mom_30b_gt1pc", text)
        self.assertIn("dip_30b_lt1pc", text)
        self.assertIn("tested_pass", text)
        self.assertIn("How to evaluate after merge", text)
        self.assertIn("Do not cull existing champions", text)
        self.assertIn("Do not retest parked fails", text)
        ...
        readme = (REPO / "README.md").read_text()
        self.assertIn("1% grind", readme)
        self.assertIn("short-MA 3-atoms", readme)
        runbook = (REPO / "docs" / "WINDOWS_DISCOVERY.md").read_text()
    ...
    def test_amendment_2026_09_13_htf_dense_mom_before_dip(self):
        ...
        self.assertIn("h4_ema_abv_24&" + REGIME_MOM_BASES[0], names)
        first_two_atom_mom = next(
            n for n in names if n.count("&") == 1 and any(
                n.endswith("&" + m) for m in REGIME_MOM_BASES
            )
        )
        first_two_atom_dip = next(
            n for n in names if n.count("&") == 1 and n.endswith("&" + REGIME_DIP_BASES[0])
        )
        self.assertLess(names.index(first_two_atom_mom), names.index(first_two_atom_dip))
        self.assertLess(
            names.index("h4_ema_abv_24&mom_18b_gt4pc"),
            names.index("h4_ema_abv_24&dip_6b_lt2pc"),
        )
        self.assertLessEqual(len(names), 8000)
        blob = " ".join(names)
    ...
    def test_amendment_2026_09_13_htf_ema_abv_18_36(self):
        ...
        self.assertIn("h4_ema_abv_24&mom_18b_gt2pc", names)
        self.assertLess(
            names.index("h4_ema_abv_24&mom_18b_gt2pc"),
            names.index("h4_ema_abv_24&dip_24b_lt5pc"),
        )
        self.assertLessEqual(len(names), 8000)
        blob = " ".join(names)
        for needle in ("wt_cross", "mfi_", "engulfing", "hammer", "doji"):
    ...
    def test_amendment_2026_09_13_h1_sma_twins_dip_priority(self):
        from hedge_fund.trading.constants import (
            MIN_BACKTEST_SHARPE,
            MIN_BACKTEST_TRADES,
            QUAL_N_WINDOWS,
        ...
        self.assertEqual(REGIME_DIP_BASES[:3], REGIME_DIP_PRIORITY)
        ...
        self.assertEqual("h1_sma_abv_24&mom_18b_gt2pc".count("&"), 1)
        self.assertNotIn("h1_sma_abv_24&mom_18b_gt2pc&don_hi_12", names)
        self.assertLess(
            names.index("h1_sma_abv_24&dip_24b_lt5pc"),
            names.index("h1_sma_abv_24&dip_24b_lt6pc"),
        )
        self.assertLess(
            names.index("h1_sma_abv_24&dip_24b_lt6pc"),
            names.index("h1_sma_abv_24&dip_18b_lt2pc"),
        )
        self.assertLess(
            names.index("h1_sma_abv_24&dip_18b_lt2pc"),
            names.index("h1_sma_abv_24&dip_6b_lt2pc"),
        )
        self.assertNotIn("h1_sma_abv_24&dip_24b_lt4pc", names)
        self.assertLess(
            names.index("h1_sma_abv_24&mom_18b_gt2pc"),
            names.index("h1_sma_abv_24&dip_24b_lt5pc"),
        )
        self.assertLessEqual(len(names), 8000)
        blob = " ".join(names)
        ...
        text = (REPO / "PROTOCOL.md").read_text()
        self.assertIn("h1_sma_abv_20", text)
        self.assertIn("h1_sma_abv_24", text)
        self.assertIn("h1_sma_abv_30", text)
        self.assertIn("REGIME_DIP_PRIORITY", text)
        self.assertIn("dip_24b_lt5pc", text)
        self.assertIn("dip_24b_lt6pc", text)
        self.assertIn("2-atom only", text)
        self.assertIn("OOS **thresholds** are unchanged", text)
        self.assertIn("No named candlesticks", text)
        workflow = (REPO / "docs" / "WORKFLOW.md").read_text()
        self.assertIn("h1_sma_abv_20", workflow)
        self.assertIn("h1_sma_abv_24", workflow)
        self.assertIn("h1_sma_abv_30", workflow)
        self.assertIn("dip_24b_lt5pc", workflow)
        self.assertIn("2-atom only", workflow)
        self.assertIn("mom-before-dip", workflow)
    ...
    def test_amendment_2026_09_14_winner_combo_mint(self):
        from hedge_fund.trading.constants import (
            DISCOVERY_REFILL_BATCH_SIZE,
            MIN_BACKTEST_SHARPE,
            MIN_BACKTEST_TRADES,
        ...
        self.assertIn("h4_ema_abv_20&mom_54b_gt2pc", names)
        ...
        self.assertNotEqual(
            near_duplicate_key("mom_54b_gt2pc"),
            near_duplicate_key("mom_48b_gt2pc"),
        )

        text = (REPO / "PROTOCOL.md").read_text()
        self.assertIn("Amendment 2026-09-14 — combine winning HTF×mom", text)
        self.assertIn("REGIME_ADMIT_ATOMS", text)
        self.assertIn("MOM_FILTERS_HTF_EXPAND", text)
        self.assertIn("OOS **thresholds** are unchanged", text)
        self.assertIn("No named candlesticks", text)
        self.assertIn("STRUCTURE_NS", text)
        workflow = (REPO / "docs" / "WORKFLOW.md").read_text()
        self.assertIn("3-atoms first", workflow)
        self.assertIn("mild_dip", workflow)
        self.assertIn("MOM_FILTERS_HTF_EXPAND", workflow)
        self.assertIn("2-atom only", workflow)
        self.assertIn("no structure and", workflow.lower())
        self.assertIn("mom-before-dip", workflow)
    ...
    def test_amendment_2026_09_14_fresh_undry_mint(self):
        from hedge_fund.trading.constants import (
            DISCOVERY_REFILL_BATCH_SIZE,
            MIN_BACKTEST_SHARPE,
            MIN_BACKTEST_TRADES,
        ...
        for name in names:
            for _atom, n in structure_lookbacks(name):
                self.assertLessEqual(n, 96, msg=name)
        for atom in (*REGIME_ATOMS_FRESH, *MOM_FILTERS_HTF_FRESH, *FRESH_CONT_ATOMS, FRESH_RSI):
            self.assertTrue(name_is_parseable(atom), msg=atom)
            parse_strategy(atom)
        combo = "h1_ema_abv_20&mom_18b_gt2pc&sma_abv_40"
        self.assertTrue(name_is_parseable(combo))
        parse_strategy(combo)
        ...
        self.assertNotEqual(
            near_duplicate_key("mom_78b_gt2pc"),
            near_duplicate_key("mom_72b_gt2pc"),
        )
        self.assertNotEqual(
        ...
        text = (REPO / "PROTOCOL.md").read_text()
        self.assertIn("Amendment 2026-09-14 — un-dry mint", text)
        self.assertIn("REGIME_ATOMS_FRESH", text)
        self.assertIn("MOM_FILTERS_HTF_FRESH", text)
        self.assertIn("FRESH_STACK_REGIME", text)
        self.assertIn("OOS **thresholds** are unchanged", text)
        self.assertIn("No named candlesticks", text)
        self.assertIn("STRUCTURE_NS", text)
        workflow = (REPO / "docs" / "WORKFLOW.md").read_text()
        self.assertIn("fresh never-tested", workflow.lower())
        self.assertIn("sma_abv_30", workflow)
        self.assertIn("MOM_FILTERS_HTF_FRESH", workflow)
        self.assertIn("2-atom only", workflow)
        self.assertIn("mom-before-dip", workflow)
        readme = (REPO / "README.md").read_text()
    ...
    def test_amendment_2026_09_14_island_undry_mint(self):
        from hedge_fund.trading.constants import (
            DISCOVERY_REFILL_BATCH_SIZE,
            MIN_BACKTEST_SHARPE,
            MIN_BACKTEST_TRADES,
        ...
        for name in names:
            for _atom, n in structure_lookbacks(name):
                self.assertLessEqual(n, 96, msg=name)
        for atom in (*UNDRY_CONT_ATOMS, UNDRY_RSI, *MOM_FILTERS_HTF_INTERMEDIATE, "h1_sma_abv_70"):
            self.assertTrue(name_is_parseable(atom), msg=atom)
            parse_strategy(atom)
        self.assertNotEqual(near_duplicate_key("sma_abv_40"), near_duplicate_key("sma_abv_30"))
        self.assertNotEqual(near_duplicate_key("sma_abv_40"), near_duplicate_key("sma_abv_50"))
        self.assertNotEqual(near_duplicate_key("rsi_14_>60"), near_duplicate_key("rsi_14_>55"))
        self.assertNotEqual(
        ...
        drained = [n for n in names if not (set(n.split("&")) & new_atoms)]
        ...
        text = (REPO / "PROTOCOL.md").read_text()
        self.assertIn("Amendment 2026-09-14 — un-dry mint: admit-island densify", text)
        self.assertIn("UNDRY_CONT_ATOMS", text)
        self.assertIn("sma_abv_40", text)
        self.assertIn("rsi_14_>60", text)
        self.assertIn("MOM_FILTERS_HTF_INTERMEDIATE", text)
        self.assertIn("OOS **thresholds** are unchanged", text)
        self.assertIn("No named candlesticks", text)
        self.assertIn("STRUCTURE_NS", text)
        workflow = (REPO / "docs" / "WORKFLOW.md").read_text()
        self.assertIn("sma_abv_40", workflow)
        self.assertIn("rsi_14_>60", workflow)
        self.assertIn("MOM_FILTERS_HTF_INTERMEDIATE", workflow)
        self.assertIn("mom-before-dip", workflow)
        readme = (REPO / "README.md").read_text()
