...
names from the active pool only (no trade-DB delete). GET
`/api/discovery/summary` farm status stays public. GET `/api/champions`
stays read-only.
...
- Ingest, farm Start/Stop, and champion retain / cull_undated are
  fail-closed: if `PAPER_DISCOVERY_INGEST_TOKEN` is unset, those POSTs
  return 503. Wrong or missing token → 401. Set the k8s secret
  `paper-discovery-ingest` on the web container. Never put the token in
  the frontend build. See [WINDOWS_DISCOVERY.md](WINDOWS_DISCOVERY.md).
