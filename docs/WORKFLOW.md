...
|---|---|---|
| **Mint** | `hedge_fund/trading/refill.py` on the farm | Bounded DIP/MOM + structure AND recipe (`STRUCTURE_NS` through **96** — no `don_hi` / `don_lo` / `near_swing_*` / `dbl_bot` `N>96`), plus densified HTF buyer-regime ANDs (`h1_ema_abv_{12,15,18,20,24,30,36,40,50,60,70}` / `h1_sma_abv_{12,15,20,24,30,36,40,50,60,70}` / `h4_ema_abv_{12,15,20,24,30,36,40,48,60}` / `h4_sma_abv_{12,15,20,24,30,36,40,50}`). **Fresh never-tested families first**: admit-island densify (`sma_abv_40` / `ema_abv_40`, `rsi_14_>60`, `MOM_FILTERS_HTF_INTERMEDIATE` on paid-off `h1_*_abv_{20,24,30,50,60}` spines, `h1_sma_abv_70`) then drained unused HTF/mom (`REGIME_ATOMS_FRESH` / `MOM_FILTERS_HTF_FRESH`) and winner-shaped stacks (REGIME+MOM × `sma_abv_30` / `ema_abv_30` × rsi, plus `rsi_14_>55`). Then drained admit-island **3-atoms first**: `regime&mom&sma_abv_50` / `ema_abv_20` (not HTF×mom×structure; recipe no longer ANDs `mom_gt` with `mild_dip`). Then **4–7 atom role-bucket** stacks on that island are continuation / RSI only (depth 4–5; REGIME+MOM spine, 0–2 continuation, 0–1 RSI; never a dip on that spine; DEEP includes `h1_ema_abv_60` / `h1_sma_abv_50` / `h1_sma_abv_60`). Cheap `near_swing_hi` N≤48 only at depth 7 was dip-gated and is not minted. Then HTF×mom 2-atom (`regime&mom`, including `MOM_FILTERS_HTF_DENSE` / `MOM_FILTERS_HTF_EXPAND` / `MOM_FILTERS_HTF_FRESH`) — 2-atom only vs expensive structure — then HTF×dip 2-atom only. Mom-before-dip: `mom_18b_gt2pc` first among regime mom bases, `dip_24b_lt5pc` / `dip_24b_lt6pc` / `dip_18b_lt2pc` first among regime dip bases (`REGIME_DIP_PRIORITY`). Central guard (`name_has_mom_gt_and_dip`) drops any `mom_*_gt*` ∧ `dip_*` stack. When never-tested leftovers run dry, the next handful is appended to `state/discovery_extended.json`. Static `generate_universe()` stays inside the ~40–120 compiled-list band. |
...
`h1_sma_abv_{12,50,60,70}` / `h4_ema_abv_{15,40,60}` /
...
(undry admit-island densify: `sma_abv_40` / `ema_abv_40`
× `rsi_14_>60`, `MOM_FILTERS_HTF_INTERMEDIATE` on paid-off
20/24/30/50/60 spines, `h1_sma_abv_70`) then drained winner-shaped 3–5:
`sma_abv_30` / `ema_abv_30` × rsi, `rsi_14_>55`) then admit-island **3-atoms first**
(`regime&mom&sma_abv_50` / `ema_abv_20`; recipe no longer ANDs
`mom_gt` with `mild_dip`)
then **4–7 atom role-bucket stacks** on that island (depth 4–5
continuation / RSI; spine REGIME+MOM; never a dip on that spine;
cheap `near_swing_hi` N≤48 only at depth 7 was dip-gated and is
not minted)
...
AND onto every HTF tag. Central guard drops `mom_*_gt*` ∧ `dip_*`.
OOS gates are unchanged.
